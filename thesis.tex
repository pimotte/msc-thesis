...
\phantom{niets} & \phantom{niets} \\
$\;$ & \\
\large{December 2016} & \large{Delft, the Netherlands}
\end{tabular}
\end{center}
\newpage
...
In this thesis, the problem of estimating trust in distributed systems is considered.
Distributed systems can be virtual or real-world systems in which multiple agents interact.
One of the biggest problems in distributed systems is that they only function well if
everyone contributes some resources. If agents do not participate, or try to pretend they
participate, but in fact are slacking off, then the system might not achieve its desired purpose.
This work presents two methods to estimate how well
agents contribute to the network, while preventing cheating. 
Firstly, the NetFlow algorithm, which uses max-flow computations
to bound the profit of cheating by Sybil attacks. Secondly, the Temporal PageRank
algorithm, which uses information about the ordering of the interactions to provide
a robust mechanism to determine reputation. Theoretical guarantees about several aspects
of these algorithms are provided. Furthermore, these mechanisms are tested on
data from a real-world distributed system: Tribler, an anonymous peer-to-peer downloading
system. Finally, taking both the theoretical and practical results, the broader implications
of these mechanisms and future possibilities are explored.

\clearpage
\chapter*{\centering \begin{normalsize}Preface\end{normalsize}}

This thesis is the capstone to my master Applied Mathematics at TU Delft.
As this final project comes to an end, I am very satisfied with
the work that follows. While I do not feel this is necessarily my best
mathematical work in the pure sense, I am happy to have been able to apply
my mathematical knowledge to the field of distributed systems and specifically
to one system which supports privacy and security. I feel that these areas
are key in ensuring that the digital world remains
at least as open and free as it has been in the past decade. 

There are a few people who have supported me in this endeavour who I would like to
thank. My daily supervisor, Johan Pouwelse. His ideas and contagious enthusiasm have 
kept me inspired and motivated throughout. Dion Gijswijt, who as mathematical supervisor
has brought the necessary rigour and precision where desired. I thank both of them for
their input and I could not have wished for more balanced pair of supervisors.
I thank Pim and Ewout, fellow members of the ``MultiChain team'',
as well as office roommates Tim and Jesse for their input during sparring sessions.
Stefan and Hassan have my gratitude for their proofreading of this work. Of course, any and
all remaining flaws are my responsibility alone. 
I thank all students and staff on the 7th floor for their creation of an enjoyable
working environment, as well as pizza/gaming night. I thank all my mathematical
friends who have tolerated my defection from mathematics.

Finally, my parents, Arien and Bernard, and my sister Lidwien. They have
supported and encouraged me throughout the entirety of the journey that has 
lead me to this point. They have shown interest and encouraged me to think
critically. I thank them for all of this.
...
\footnotetext{https://www.torproject.org/about/overview.html.en}
\addtocounter{footnote}{1}
\footnotetext{https://creativecommons.org/licenses/by/3.0/us/}
...
in which Temporal PageRank is introduced, a mechanism that uses the order of interactions in the system to compute reputation.
...
\begin{definition}[Ordered interaction graph]
    ...
    \begin{equation*}
        w_{G_M}(e) := w(i, p), \mbox{ where } p \in a(i) \cap a(j), e = (v_{i}, v_{j}) \in A:w
    \end{equation*}
    ...
    \begin{figure}[h]
        \centering
         \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
            ...
            \path[draw, thick]
            (p1q1) edge node [right] {5} (p2r1) 
            (p1q1) edge node [left] {3} (q2r2) 
            (p2r1) edge node [right] {7} (p3r3) 
            (p2r1) edge node [below right] {1} (q2r2) 
            (q2r2) edge node [left] {8} (p3r3) 
            ;
         \end{tikzpicture}
...
\begin{example}[Block graph]
    In Figure~\ref{fig:ex_block} an example of a block graph can be found. 
    This representation yields a another view on the ordered interaction model.
    In this graph, the fact that each agent has their own sequence of interactions
    is stressed, with additional arcs indicating the other party involved.
    In the ordered interaction graph, there were some issues with certain contributions
    not being reflected in the graph. This issue is not present in this graph, at the
    cost of a less elegant definition. The reason for this is that
 ...
 \begin{figure}[ht]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        \path[draw, thick]
        (p1) edge node [right] {5} (p2)
        (p2) edge node [left] {7} (p3)
        (q1) edge node [left] {3} (q2)
        (r1) edge node [right] {1} (r2)
        (r2) edge node [right] {8} (r3)
        (p1) edge node [above right, pos=0.1] {3} (q2)
        (q1) edge node [above left, pos=0.1] {5} (p2)
        (p2) edge node [above right, pos=0.1] {1} (r2)
        (r1) edge node [above left, pos=0.1] {7} (p3)
        (q2) edge [bend left=60] node [right] {8} (r3)
        (p3) edge node [left] {2} (p4)
        (r3) edge node [above, pos=0.1] {2} (p4)
        (r3) edge node [left] {3} (r4)
        (p3) edge node [right, pos=0.1] {3} (r4)
        (q2) edge node [left] {3} (q3)
        (r2) edge node [above] {3} (q3)
...
\begin{figure}[h]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        \path[draw, thick]
        (p) edge [bend right=20] node [right] {5} (q)
        (q) edge [bend right=20] node [right] {3} (p)
        (p) edge [bend right=20] node [left] {9} (r)
        (r) edge [bend right=20] node [left] {4} (p)
        (q) edge [bend right=20] node [below] {3} (r)
        (r) edge [bend right=20] node [below] {8} (q)
        ;
     \end{tikzpicture}
...
\begin{figure}[h]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        \node [main node] (i) {$p$} ;
        \node [main node] (j) [right = 2cm of i] {$s$};
        \node [main node] (s1) [above right = 2cm and 2cm of j] {$s_1$};
        \node [main node] (s2) [above right = 1cm and 2cm of j] {$s_2$};
        \node [main node] (sn) [below right = 1cm and 2cm of j] {$s_n$};
        \path (s2) -- (sn) node[midway, main node](si)  {$s_k$};

        \node [red] [right = 0.1 cm of s1] {1};
        \node [red] [right = 0.1 cm of s2] {1};
        \node [red] [right = 0.1 cm of si] {1};
        \node [red] [right = 0.1 cm of sn] {1};
        \node [red] [above = 0.1 cm of j] {1};


        \path[draw, thick]
        (j) edge node [above] {1} (i)
        (s1) edge node [above] {1} (j)
        (s2) edge node [above] {1} (j)
        (sn) edge node [above] {1} (j)
        ;
...
\begin{figure}[h]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        \node [main node] (i) {$i$} ;
        \node [main node] (j) [right = 2cm of i] {$j$};
        \node [main node] (s1) [above right = 2cm and 2cm of j] {$s_1$};
        \node [main node] (s2) [above right = 1cm and 2cm of j] {$s_2$};
        \node [main node] (sn) [below right = 1cm and 2cm of j] {$s_n$};
        \path (s2) -- (sn) node[midway, main node](si)  {$s_k$};

        \node [red] [right = 0.1 cm of s1] {0};
        \node [red] [right = 0.1 cm of s2] {1};
        \node [red] [right = 0.1 cm of si] {1};
        \node [red] [right = 0.1 cm of sn] {1};
        \node [red] [above = 0.1 cm of j] {0};
        ...
        (s2) edge node [above] {1} (j)
        (sn) edge node [above] {1} (j)
        (i) edge [bend left=20] node [above] {1} (s1)
        ;
...
\begin{definition}[Sybil attack]
    ...
    $E_s = \{(x, y) : x, y \in V_s \cup J\}$, and $w_s$ are the edge weights for
...
Note that $E_s$ may include edges to identities not controlled by Sybils. In order to include these edges,
the work actually has to be performed. For edges within $V_s$,
...
\begin{definition}[Sybil attack profit]
    ...
    the network after $n$ steps, including work performed before the start of the Sybil attack.
    Let $\omega^n_+$ be the amount of work that agents in $J$ or any of their
    Sybils obtain from the network. Any work obtained before the start of the Sybil attack is
    disregarded.
...
\begin{figure}[h]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        \node [main node] (p) {P} ;
        \node [main node] (q) [below right = 3.46cm and 2cm] {Q} ;
        \node [main node,fill=gray] (r) [below left = 3.46cm and 2cm] {R} ;
        \node [main node] (t) [right = 3.46cm of p] {T};

        \path[draw, thick]
        (p) edge [bend right=20] node [right] {5} (q)
        (q) edge [bend right=20] node [right] {3} (p)
        (p) edge [bend right=20] node [left] {9} (r)
        (r) edge [bend right=20] node [left] {6} (p)
        (q) edge [bend right=20] node [below] {3} (r)
        (r) edge [bend right=20] node [below] {8} (q)
        (t) edge node [above] {2} (p)
        (t) edge node [below = 0.4 cm] {2} (q)
        ;
     \end{tikzpicture}
...
\begin{figure}[h]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        \node [ForestGreen] [above = 0.3 cm of p] {$12 - 9 = 3$};
        \node [red] [below = 0.3 cm of q] {$6 - 12 = -6$};
        \node [ForestGreen] [right = 0.3 cm of t] {$4 - 0 = 4$};
        ...
        ;
     \end{tikzpicture}
     \caption{The first step of a NetFlow computation}
     \label{fig:ex_nf_1}
\end{figure}

The second step (Figure~\ref{fig:ex_nf_2}) in the NetFlow algorithm involves flows from the other agents towards the calculating
agent, respecting the node capacities calculated in the first step. This yields the scores for each
agent. Agent $P$ is the highest ranked agent with a score of 3, then $T$ with a score of $2$, followed
by $Q$ with a score of $0$. Note that purely based on the contributions from a neutral perspective,
one might rate agent $T$ as having contributed more than agent $P$, but due to the structure of the
interaction graph, this is not the case. Because agent $T$ is farther away in the graph and has
no direct interactions with $R$ this is a defensible result.
...
\begin{figure}[h]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        \node [ForestGreen] [above = 0.3 cm of p] {$3\mbox{ }( \mbox{cap: } 3)$};
        \node [red] [below = 0.3 cm of q] {$0\mbox{ }(\mbox{cap: } 0)$};
        \node [ForestGreen] [right = 0.3 cm of t] {$2\mbox{ }(\mbox{cap: } 4)$};
...
\begin{example}[Secondary effects of scaling]
    ...
    \begin{figure}[h]
        ...
        \node [red] [below left = 0.1cm and 0.1cm of q] {100-100=0} ;
        \node [red] [below left = 0.1cm and 0.1cm of r] {2-2=0} ;
        \node [blue] [below right = 0.1cm and 0.1cm of r] {2-1=1} ;
        \node [blue] [below right = 0.1cm and 0.1cm of q] {100-50=50} ;

        \path[draw, thick]
        (p) edge [bend right=20] node [right] {100} (q)
        (q) edge [bend right=20] node [right] {100} (p)
        (p) edge [bend right=20] node [left] {2} (r)
        (r) edge [bend right=20] node [left] {2} (p)
        ;
...
\begin{example}[Lack of weak transitive trust in Temporal PageRank]
     To further illustrate Theorem~\ref{thm:no_trans_trust}, a small example. Consider
     the block graph in Figure~\ref{fig:ex_no_tt}. Computing from perspective of $p$ leads
     to the score of $0.246$ for $q$ and $r$. 

     If agent $q$ then performs work for $p$, and $k$ performs work for $q$, the result
     will be as in Figure~\ref{fig:ex_no_tt_2}. Here, the score of $k$ is not surpass the
     score of $r$, which means that the definition for transistive trust is not satisfied.
     The result is symmetrical with respect to $r$ and $q$, so the property of weak transitive
     trust is not satisfied.
     ...
     \begin{figure}[ht]
        \centering
         \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
            ...
            \node [main node] (q2) [below = 1.5cm of q1] {q2} ;
            \node [main node] (p4) [below = 1.5cm of p3] {p4} ;
            \node [main node] (q3) [below = 2.5cm of q2] {q3} ;
            \node [main node] (p5) [below = 1.5cm of p4] {p5} ;
            \node [main node] (r3) [below = 2.5cm of r2] {r3} ;

            \node [red] (ps) [below = 0.1cm of p5] {0.508};
            \node [red] (qs) [below = 0.1cm of q3] {0.246};
            \node [red] (rs) [below = 0.1cm of r3] {0.246};

            \path[draw, thick]
            (p1) edge node [right] {0} (p2)
            (p2) edge node [left] {0} (p3)
            (q1) edge node [left] {1} (q2)
            (r1) edge node [right] {1} (r2)
            (p1) edge node [above left, pos=0.1] {1} (q2)
            (q1) edge node [above right, pos=0.1] {0} (p2)
            (p2) edge node [above right, pos=0.1] {1} (r2)
            (r1) edge node [above left, pos=0.1] {0} (p3)
            (q2) edge node [left] {1} (q3)
            (p3) edge node [left] {0} (p4)
            (q2) edge node [right, pos=0.1] {0} (p4)
            (p3) edge node [above left, pos=0.01] {1} (q3)
            (r2) edge node [right] {1} (r3)
            (p4) edge node [left] {0} (p5)
            (r2) edge node [left, pos=0.1] {0} (p5)
            (p4) edge node [above right, pos=0.01] {1} (r3)
            ;
         \end{tikzpicture}
         \caption{Block graph. In red, the Temporal PageRank scores of each agent, when $p$ computes.}
         \label{fig:ex_no_tt}
    \end{figure}
     ...
     \begin{figure}[ht]
        \centering
         \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
            ...
            \node [main node] (p5) [below = 1.5cm of p4] {p5} ;
            \node [main node] (r3) [below = 2.5cm of r2] {r3} ;
            \node [main node] (p6) [below = 1.5cm of p5] {p6} ;
            \node [main node] (q4) [below = 2cm of q3] {q4} ; 
            \node [main node] (k1) [left = 1.5cm of q4] {k1} ; 
            \node [main node] (k2) [below = 1.5cm of k1] {k2} ; 
            \node [main node] (q5) [below = 1.5cm of q4] {q5} ; 

            \node [red] (ps) [below = 0.1cm of p6] {0.403};
            \node [red] (qs) [below = 0.1cm of q5] {0.310};
            \node [red] (rs) [below = 0.1cm of r3] {0.163};
            \node [red] (rk) [below = 0.1cm of k2] {0.125};
            ...
            (r2) edge node [left, pos=0.1] {0} (p5)
            (p4) edge node [above right, pos=0.01] {1} (r3)
            (q3) edge node [left] {100} (q4)
            (p5) edge node [right] {0} (p6)
            (q3) edge node [right, pos=0.1] {0} (p6)
            (p5) edge node [below, pos=0.9] {100} (q4)
            (k1) edge node [left] {100} (k2)
            (q4) edge node [left] {0} (q5)
            (k1) edge node [right, pos=0.1] {0} (q5)
            (q4) edge node [above left, pos=0.01] {100} (k2)
...
\begin{figure}[ht]
    \centering
    \subfloat{\includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow0_7}}
    \subfloat{\includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow0_8}}\\
    \addtocounter{subfigure}{-1}
    \addtocounter{subfigure}{-1}

    \subfloat{\includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow1_7}}
    \subfloat{\includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow1_8}}\\

    \addtocounter{subfigure}{-1}
    \addtocounter{subfigure}{-1}

    \subfloat[][Computing agent has upload and download \\ around 120MB, for $\alpha \in \{1,2,4\}$]{\includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow2_7}}
    \subfloat[][Computing agent has upload and download \\ around 1050MB, for $\alpha \in \{1,2,4\}$]{\includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow2_8}}\\
...
\begin{figure}[ht]
    \ContinuedFloat
    \centering
        \subfloat{        \includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow0_9}}
        \subfloat{        \includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow0_10}} \\

        \subfloat{        \includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow1_9}} 
        \subfloat{        \includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow1_10}} \\
        ...
        \subfloat[][Computing agent has upload and download \\ around 1400MB, for $\alpha \in \{1,2,4\}$]{        \includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow2_9}} 
        \subfloat[][Computing agent has upload and download \\ around 100GB, for $\alpha \in \{1,2,4\}$]{        \includesvg[width=0.47\textwidth]{plots_all/plot_up_down_size_flow2_10}} \\
