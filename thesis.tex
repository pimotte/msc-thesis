...
\begin{example}[Block graph]
    In Figure~\ref{fig:ex_block} an example of a block graph can be found. This graph has a similar structure
    as the ordered interaction graph, except that each interaction is split into the two parts belonging to each respective
    party. This representation yields a different view on the ordered interaction model.
 ...
 \begin{figure}[ht]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        (p1) edge node [above left, pos=0.1] {3} (q2)
        (q1) edge node [above right, pos=0.1] {5} (p2)
        (p2) edge node [above right, pos=0.1] {1} (r2)
        (r1) edge node [above left, pos=0.1] {7} (p3)
        (q2) edge [bend right] node [left] {8} (r3)
