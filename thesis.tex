...
that do not possess the entire file can request little pieces of it from other
...
\musthave{Something about reputation. Link with trust. Make introduction more matching to RQ}
...
as sybil attacks in broader literature. \nicetohave{Reference}
...
theoretical worst-case performance is useable. It might be that some
...
\nicetohave{Some sort of practical bounds}
 ...
 \begin{figure}[ht]
    \centering
     \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
        ...
        (q1) edge node [below left, pos=0.9] {5} (p2)
        (p2) edge node [below left, pos=0.9] {1} (r2)
...
Definition~\ref{def:choice_set} is as Seuken~and~Parkes originally formulated it. For compatibility reason,
...
Definition~\ref{def:SWTA} defines a policy to choose an agent to receive resources given scores
...
interaction graph, this is not the case. 
...
still vulnerable to weakly-beneficial sybil attacks, since those are defined quite widely and
a weakly-beneficial sybil attack does not necessarily have to be detrimental to the network,
...
\begin{definition}[Temporal PageRank]
    ...
    The score of a half-interaction is it's probability in the stationary distribution of this random walk.
...
the network. A database dump from this crawler has been acquired on the 31st of August. The software
...
since its resistance against sybil attacks is not quite as strong as NetFlows, more research could be done
