...
\framebox[16cm]{\raisebox{0cm}[1.5cm][1.5cm]
 {\textbf{
   ...
   \begin{center}
       ...
       %Transitive trust in distributed systems with sybil resilliance.
       %Graph-based, sybil-resilliant trust mechanisms in distributed systems
   \end{center}
   \end{minipage}}}}
 ...
 \begin{figure}[b]
        \centering
         \begin{tikzpicture}[->,thick,scale=0.8,main node/.style={circle, draw, fill=white,
            ...
            \node [main node] (a1) {1} ;
            \node [main node] (a2) [right = 5cm of a1] {2} ;
            \node [main node] (a3) [below = 3cm of a1] {3} ;
            \node [main node] (a4) [right = 5cm of a3] {4} ;

            \node (l2) [right = 0.5cm of a2] {$9 - 5 = 4$};
            \node (l3) [left = 0.5cm of a3] {$6 - 11 = -5$};
            \node (l4) [right = 0.5cm of a4] {$0 - 0 = 0$};

            \path[draw, thick]
            (a1) edge [bend right] node [below] {5} (a2) 
            (a2) edge [bend right] node [above] {4} (a1) 
            (a1) edge [bend right] node [left] {6} (a3)
            (a3) edge [bend right] node [right] {6} (a1)
            (a2) edge [bend left=10] node [right = 0.1cm ] {5} (a3)
            (a3) edge node [below] {10} (a4)
            ;
         \end{tikzpicture}
...
\begin{theorem}[Properties and robustness of NetFlow]
    NetFlow as accounting mechanism, with SWTA as allocation policy
...
%TODO: Collusion attack. Is this an attack?
...
has implemented the MultiChain data structure into Tribler. In collaboration with Bongers and Veldhuisen,
